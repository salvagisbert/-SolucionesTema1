...
Encontramos la diferencia en los conjuntos en los que se aplica la operación.
...
#El número de librerías instaladas.
length(libreturn$results)#564
...
```
Cumplen la condición porque se adecuan al modelo.
...
ggplot(cabbages, aes(VitC,fill = Cult)) + geom_histogram(alpha=0.5)#el gráfico ggplot nos compara gráficamente las dos variedades

```

Observamos que el cultivo c52 tien más vitamina c
...
plot(cabbages$VitC ~ cabbages$HeadWt) #comparacion **VitC** en función del peso de la calabaza **HeadWt**
...
plot(cabbages$VitC ~ cabbages$HeadWt, pch=c(1,18)[cabbages$Cult])#comparacion **VitC** en función del peso de la calabaza **HeadWt** por  variedad de calabaza


Modeloc39<-lm(cabbages$VitC ~ cabbages$HeadWt, data=cabbages, subset=which(Cult=="c39"))#obtenemos el modelo c39
Modeloc52<-lm(cabbages$VitC ~ cabbages$HeadWt, data=cabbages, subset=which(Cult=="c52"))#obtenemos el modelo c52
